...
int main(){
	...
	if((script = popen(" mkdir -p Imagens_Thread Imagens_Serial Tempos_Serial Tempos_Thread Imagens_Thread/Linhas Imagens_Thread/Colunas ", "w")) == NULL){
		perror("popen");
		exit(1);
	}
	pclose(script);
...
/*---------------------------------LOOP PARA AS OPERAÇÕES THREADS LINHAS---------------------------------*/
	...
	for(k = 1; k <=10; k++){
		...
		fprintf(script, "threads%d.jpg = demo.jpg * 3.0 1 0 %d ", k, k);
		pclose(script);
	}
	...
	if((script = popen("for i in $(seq 1 10); do cat threads$i.jpg | mv threads$i.jpg Imagens_Thread/Linhas; done", "w")) == NULL){
		perror("popen");
		exit(1);
	}
	pclose(script);
...
/*---------------------------------LOOP PARA AS OPERAÇÕES THREADS COLUNA---------------------------------*/
	...
	for(k = 1; k <=10; k++){
		...
		fprintf(script, "threads%d.jpg = demo.jpg * 3.0 1 1 %d ", k, k);
		pclose(script);
	}
	...
	if((script = popen("for i in $(seq 1 10); do cat threads$i.jpg | mv threads$i.jpg Imagens_Thread/Colunas; done", "w")) == NULL){
		perror("popen");
		exit(1);
	}
	pclose(script);
